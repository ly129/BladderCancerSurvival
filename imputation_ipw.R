...
covs <- subset(dd,
               select = c(Gender, Race, StageAtDiagnosis, AgeAtDiagnosis, EcogValue,
                          ...
                          AdvancedDiagnosisYear, DiagnosisPeriod))
...
tab
bal.plot(ipw, var.name = "AgeAtDiagnosis", which = "both")
bal.plot(ipw, var.name = "StageAtDiagnosis", which = "both")
love.plot(ipw)
...
# 
# for (i in 1:5) {
#   tmp <- complete(imp, action = 1)
#   
#   ipw <- mnps(DiagnosisPeriod ~ Gender + Race + StageAtDiagnosis + AgeAtDiagnosis +
#                 EcogValue + SmokingHistory + Location + Insurance + PracticeType +
#                 PrimarySite + Surgery + Chemotherapy + Immunotherapy + AntibodyConjugate,
#               data = tmp,
#               stop.method = "es.max",
#               n.trees = 5000)
#   plot(ipw, plots = 1)
