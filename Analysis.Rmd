...
## 2.1 First line treatment start date bewteen 01/01/2011 and 09/30/2023
...
## 4.2 Analysis
```{r crudeanalysis, fig.dim=c(8,6), warning=FALSE}
auc.surv <- with(auc,
...
##### Survival curve
```{r wtsmean, fig.dim=c(7,6)}
wts1.mean <- rowMeans(wts1)
# auc$Line1StartPeriod <- as.character(dd$Line1StartPeriod)

pdf(file = "survival_plot.pdf", width = 7, height = 6)
plt <- ggsurvplot(fit = with(auc, survfit(Surv(time = as.numeric(CensoringDate - Line1StartDate),
                                               event = Death) ~ Line1StartPeriod,
                  ...
                  legend.labs = c("Before ICI Approval\n(2011-2016)", "After ICI and Before ADC Approval\n(2017-2019)", "After ADC Approval\n(2020-2023)"))
plt$plot <- plt$plot + 
  geom_text(x = 5.2 * 365,
            y = 0.78,
            label = "3-Year Survival Probabilities", hjust = "left") + 
  geom_text(x = 5.5 * 365,
            y = 0.72,
            label = "Before ICI: 19.2% (17.5%, 20.9%)", colour = "#E7B800", hjust = "left") + 
  geom_text(x = 5.5 * 365,
            y = 0.66,
            label = "After ICI and Before ADC: 23.3% (21.6%, 24.9%)", colour = "#2E9FDF", hjust = "left") + 
  geom_text(x = 5.5 * 365,
            y = 0.60,
            label = "After ADC: 27.6% (25.7%, 29.5%)", colour = "red", hjust = "left")
plt
dev.off()
```
...
tab1 <- svyCreateTableOne(vars = c("AgeAtLine1" ,
                                   "BirthSex" , "Race" , "StageAtDiagnosis" ,
                                   "EcogValueAtLine1" ,
                                   "SmokingHistory" , "Location" ,
                                   "Insurance" , "PracticeType" , "PrimarySite"),
                          strata = "Line1StartPeriod", data = aucsvy, smd = TRUE, test = FALSE)
print(tab1, smd = TRUE, showAllLevels = TRUE)
write.csv(print(tab1, smd = TRUE, showAllLevels = TRUE), file = "adjustedtable1.csv")
```
...
ggsurvplot(fit = fit.ipw.age,
           ...
           censor = FALSE, fontsize = 3,
           legend.title = "Initiation Period",
           legend.labs = c("Before ICI Approval", "After ICI and Before ADC Approval", "After ADC Approval", "Before ICI Approval2", "After ICI and Before ADC Approval2", "After ADC Approval2", "Before ICI Approval3", "After ICI and Before ADC Approval3", "After ADC Approval3"))
```
...
ggsurvplot(fit = fit.ipw.bsex,
           ...
           censor = FALSE, fontsize = 3,
           legend.title = "Initiation Period",
           legend.labs = c("Before ICI Approval", "After ICI and Before ADC Approval", "After ADC Approval", "Before ICI Approval2", "After ICI and Before ADC Approval2", "After ADC Approval2"))
```
